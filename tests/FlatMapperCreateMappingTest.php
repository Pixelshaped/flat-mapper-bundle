...
#[CoversMethod(FlatMapper::class, 'setValidateMapping')]
...
class FlatMapperCreateMappingTest extends TestCase
{
    ...
    public function testCreateMappingWithCacheServiceDoesNotAssert(): void
    {
        ...
        $reflectionMethod = (new \ReflectionClass(MappingResolver::class))->getMethod('createMappingRecursive');
        $reflectionMethod->setAccessible(true);
        $cacheInterface = $this->createMock(CacheInterface::class);
        $cacheInterface->expects($this->once())->method('get')->willReturn(
            $reflectionMethod->invoke($mappingResolver, AuthorDTO::class)
        );
    ...
    public function testCreateMappingWithValidationDisabledDoesNotAssert(): void
    {
        $flatMapper = new FlatMapper();
        $flatMapper->setValidateMapping(false);

        $this->expectNotToPerformAssertions();
        $flatMapper->createMapping(RootDTOWithNoIdentifier::class);
    }
